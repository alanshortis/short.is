...
Oh My Zsh includes a few, e.g. `g` being short for `git` and `gco` being short for `git checkout`. To add your own, open `.zshrc` from your user folder (you'll need to open it from finder, and you will have needed to already have made hidden files visible) and at the bottom of the file there are some examples you can edit or replace.
