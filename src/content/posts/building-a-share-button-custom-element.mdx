...
I have [complained before](/writing/the-ethics-of-standards) about the tendency for product teams to build custom user interfaces that are worse than the native equivalent in every possible way. The [web share API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Share_API) and [custom elements](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_custom_elements) are platform features that can be combined to build a share component with the best possible UX, minimal code, and progressive enhancement.

There are some examples in this post that depend on the web share API. If your browser doesn't support it, you may need to come back in a [browser that does](https://caniuse.com/web-share).
...
Before React and components, concerns were separated by technology:
...
> Styles can be scoped to a custom element via [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM), another part of the web components spec, which is not covered here. Styles added in a custom element as they are here are global. This is not a great custom element.
...
This example barely scratches the surface of what custom elements and the wider web components spec can provide. Some of the approaches here don't scale as well as any large app would require, but the web share API's particular requirements for browser support and implementation make this a good-enough tiny component. There is room for improvement; adding social media share buttons and making a fully featured share menu that includes web share but doesn't depend on it would be the way to go.

If I were to be at the beginning of a project to build a design system for a large team of web engineers, I would give web components some serious consideration. They're lightweight, fast, and can be used anywhere, and they're not as weird as they seem at first glance.
